...
// 原站公开解析接口无需登录;第三方解析器仅作为备用回退。
const DEFAULT_RESOLVER_RESOLVE_PATH = "/api/v/resolve";
const DEFAULT_RESOLVER_FALLBACK_ORIGIN = "https://javstrm.emby-59f.workers.dev";
const DEFAULT_RESOLVER_FALLBACK_RESOLVE_PATH = "/api/resolve";
...
function resolverFallbackOrigin(env) {
  return String(env.JAVSTRM_FALLBACK_ORIGIN || DEFAULT_RESOLVER_FALLBACK_ORIGIN).replace(/\/$/, "");
}

function resolverFallbackResolvePath(env) {
  const value = String(env.JAVSTRM_FALLBACK_RESOLVE_PATH || DEFAULT_RESOLVER_FALLBACK_RESOLVE_PATH).trim();
  if (!value) return DEFAULT_RESOLVER_FALLBACK_RESOLVE_PATH;
  return value.startsWith("/") ? value : `/${value}`;
}
...
async function javdbRequest(path, env, fetchImpl, options = {}) {
  ...
  let response = await send(Boolean(options.token));
  let payload = await response.clone().json().catch(() => ({}));
  if (options.token && (!response.ok || (payload.success !== undefined && payload.success !== 1))) {
    try {
...
async function resolverJson(path, env, fetchImpl, origin = resolverOrigin(env)) {
...
async function resolveVideo(movie, env, fetchImpl) {
  ...
  const query = `?code=${encodeURIComponent(code)}&lang=zh`;
  const candidates = [
    { origin: resolverOrigin(env), path: resolverResolvePath(env) },
    { origin: resolverFallbackOrigin(env), path: resolverFallbackResolvePath(env) },
  ];
  const seen = new Set();
  let payload = null;
  let lastError = null;
  for (const candidate of candidates) {
    const key = `${candidate.origin}${candidate.path}`;
    if (!candidate.origin || seen.has(key)) continue;
    seen.add(key);
    try {
      const result = await resolverJson(
        `${candidate.path}${query}`,
        env,
        fetchImpl,
        candidate.origin,
      );
      if (sourceVariants(result).length === 0) continue;
      payload = result;
      break;
    } catch (error) {
      lastError = error;
...
async function authenticate(request, env, fetchImpl) {
  ...
  // 访客访问开启时,客户端从公开用户列表选择访客即可免密码进入。
  if (guestAccessEnabled(env) && !password && username === virtualUser(env).Name) {
    return authenticationResponse(request, env, virtualUser(env), guestToken(env));
  }
...
export async function handleEmby(request, env = {}, fetchImpl = fetch) {
  ...
  if (path === "/Users/Public") {
    if (guestAccessEnabled(env)) {
      return jsonResponse([virtualUser(env, undefined, false)]);
    }
    return jsonResponse([]);
  }
  if (path === "/Users") {
